...
func (s *server) authenticate(w http.ResponseWriter, r *http.Request) {
	...
	if s.strictSessionValidation {
		ctx := setTLSContext(context.Background(), s.caBundle)
		oauth2Token := session.Values[userSessionOAuth2Tokens].(oauth2.Token)
		_, err := s.provider.UserInfo(ctx, oauth2.StaticTokenSource(&oauth2Token))
		if err != nil {
			// TODO: Check error code for 40x
